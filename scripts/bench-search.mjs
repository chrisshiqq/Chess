...
function runSearch(depth, exactRootScores, profile, nonRootPvs, metrics, kingSafetyFastPath, verifyKingSafetyFastPath, incrementalControl, verifyIncrementalControl, lazyIncrementalControl) {
  return new Promise((resolve, reject) => {
    ...
    worker.postMessage({
      ...
      payload: {
        ...
        openingBookEnabled: false, ply: 0, enableTimeLimit: false, exactRootScores, profile, metrics, nonRootPvs, kingSafetyFastPath, verifyKingSafetyFastPath, incrementalControl, verifyIncrementalControl, lazyIncrementalControl
      }
    });
  });
}
...
function printSummary(label, run) {
  ...
  if (perf.incrementalControl) {
    console.log(`  incrementalControl enabled=${perf.incrementalControl.enabled} lazy=${perf.incrementalControl.lazy} updates=${perf.incrementalControl.updates} avgDirty=${perf.incrementalControl.averageDirtySlots} refreshes=${perf.incrementalControl.lazyRefreshes} verify=${perf.incrementalControl.verificationRuns}/${perf.incrementalControl.verificationFailures}`);
  }
  if (perf.profile) {
...
const incrementalControl = process.env.BENCH_INCREMENTAL_CONTROL === '1';
const verifyIncrementalControl = process.env.BENCH_VERIFY_INCREMENTAL_CONTROL === '1';
const lazyIncrementalControl = process.env.BENCH_LAZY_INCREMENTAL_CONTROL === '1';
const jobs = [];
...
for (const job of jobs) {
  ...
  results.push(printSummary(job.label, await runSearch(depth, job.exact, profile, nonRootPvs, metrics, kingSafetyFastPath, verifyKingSafetyFastPath, incrementalControl, verifyIncrementalControl, lazyIncrementalControl)));
}
const outPath = join(__dirname, `bench-d${depth}-${profile ? 'profile' : 'latest'}.json`);
writeFileSync(outPath, JSON.stringify({ depth, mode, profile, metrics, nonRootPvs, kingSafetyFastPath, verifyKingSafetyFastPath, incrementalControl, verifyIncrementalControl, lazyIncrementalControl, results }, null, 2));
console.log(`\nSaved JSON: ${outPath}`);
