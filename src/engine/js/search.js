...
// 行棋方：入口把 'red'/'black' 转成整数，热路径只比 0/1。
const SIDE_RED = 0;
const SIDE_BLACK = 1;
const colorToSide = (color) => (color === 'red' || color === SIDE_RED ? SIDE_RED : SIDE_BLACK);
const sideToColor = (side) => (side === SIDE_RED ? 'red' : 'black');
...
const createSearchPieceState = (board, gameStage = 'mid') => {
    ...
    return {
        // UI/点棋：runWithPieceState 用这份对象判断是否已有 state
        board,
        squareToSlot,
        squareCodes,
        rowOccupancy,
...
const evaluateBoard = (board, currentPlayer = null, gameStage = 'mid') =>
    ...
    const boardInfo = createRelationBoardInfo();
    calculateDerivedValues(
        board, piecesInfo, currentPlayer == null ? null : colorToSide(currentPlayer), boardInfo
    );
...
const evaluatePiece = (board, currentPlayer = null, gameStage = 'mid') =>
    ...
    calculateTacticalValues(
        piecesInfo, currentPlayer == null ? null : colorToSide(currentPlayer), boardInfo, board
    );
...
const filterLegalMoves = (fromSq, color, encodedMoves, wasInCheck, checkInfo) => {
    ...
    for (let i = 0; i < encodedMoves.length; i++) {
        ...
        const illegal = leavesOwnKingUnsafe(
            pieceState, color, fromSq, toSq, wasInCheck, checkInfo
        );
...
const sortRootMoves = (moves, currentPlayer, boardInfo = null, ttMove = null, killers = null, knownInCheck = null, parallelArrays = null) => {
    const __t0 = searchContext.profile ? performance.now() : 0;
...
const calculateDerivedValues = (board, piecesInfo, currentPlayer = null, boardInfo = null) => {
    ...
    if (currentPlayer != null) {
        ...
        let checkInfo = null;
        if (inCheck) {
            collectCheckersFromState(
                activePieceStateFor(board), currentPlayer, scratchLegalCheckInfo
            );
            checkInfo = scratchLegalCheckInfo;
        }
        let hasMoves = false;
        for (const info of piecesInfo) {
            if ((info.pieceCode < 8) === (currentPlayer === SIDE_RED)) {
        ...
        if (!hasMoves) {
            // 没有合法走法，检查是否被将军
            const inCheck = currentPlayer === SIDE_RED ? boardInfo.redIsInCheck : boardInfo.blackIsInCheck;
            const opponent = sideToColor(currentPlayer ^ 1);
...
const pinOwnGuardSlot = (squareCodes, squareToSlot, sq, kingIsRed, rank, file, onlySq) => {
    ...
    if ((scratchPinnedGuardBits & bit) === 0) {
        ...
        scratchPinOnlySq[slot] = -1;
        scratchPinnedGuardBits |= bit;
    }
...
const collectPinsOnRay = (
    squareCodes, squareToSlot, first, second, third,
    firstSq, secondSq, thirdSq, kingIsRed, rank, file
) => {
    if (first === 255 || second === 255) return;
    const secondCode = squareCodes[secondSq];
    if (((secondCode < 8) !== kingIsRed) && (secondCode & 7) < 3) {
        pinOwnGuardSlot(squareCodes, squareToSlot, firstSq, kingIsRed, rank, file, -1);
    }
    if (third === 255) return;
    const thirdCode = squareCodes[thirdSq];
    if (((thirdCode < 8) !== kingIsRed) && (thirdCode & 7) === 6) {
        pinOwnGuardSlot(squareCodes, squareToSlot, firstSq, kingIsRed, -1, -1, thirdSq);
        pinOwnGuardSlot(squareCodes, squareToSlot, secondSq, kingIsRed, -1, -1, thirdSq);
    }
};
...
const collectPinsFromKing = (pieceState, kingSq, kingIsRed) => {
    ...
    const squareToSlot = pieceState.squareToSlot;
    const r = SQ_ROW[kingSq];
    const c = SQ_COL[kingSq];
...
const collectPinnedGuardSlots = (pieceState, guardInterest = 0xFFFFFFFF) => {
    ...
    if ((guardInterest & pieceState.blackAliveMask) !== 0) {
        collectPinsFromKing(pieceState, pieceState.blackGeneralSq, false);
    }
    return scratchPinnedGuardBits >>> 0;
};
...
const pinnedCanGuardSquare = (slot, targetSq) => {
    ...
    if (rank >= 0 && SQ_ROW[targetSq] !== rank) return false;
    const file = scratchPinFile[slot];
    if (file >= 0 && SQ_COL[targetSq] !== file) return false;
    return true;
};
...
const markCheckerResolveSquares = (state, generalSq, checkerSq, kind, legSq, mask) => {
    ...
    const gr = SQ_ROW[generalSq];
    const gc = SQ_COL[generalSq];
    const cr = SQ_ROW[checkerSq];
    const cc = SQ_COL[checkerSq];
    const squareCodes = state.squareCodes;
    if (gr === cr) {
...
const appendValidatedStagedSpecial = (moves, move, pieceState, currentPlayer, quietOnly) => {
    ...
    if ((pieceCode < 8) !== (currentPlayer === SIDE_RED)) return false;
    const targetCode = pieceState.squareCodes[toSq];
...
const appendTrueStagedMoves = (
    ...
    if (stage === 1) {
        ...
        const pieceCodes = pieceState.pieceCodes;
        const pieceSquares = pieceState.pieceSquares;
        const n = collectOwnSlotsInScanOrder(pieceState, currentPlayer === SIDE_RED);
        for (let i = 0; i < n; i++) {
            const slot = scratchOwnScanSlots[i];
            const sq = pieceSquares[slot];
...
// pieceAtSq: 90 格 → piecesInfo；写 attack/guard/control mask
const applyRelationSquare = (squareCodes, info, pieceAtSq, tr, tc, bit, relCtx, isRed) => {
...
const fillNonCannonRelations = (squareCodes, info, pieceAtSq, relCtx) => {
    const { r, c, pieceCode } = info;
    const isRed = pieceCode < 8;
    const bit = 1 << relCtx.pieceIndex;
    const colorIdx = isRed ? 0 : 1;
    const fromSq = r * 9 + c;
    let mobilityValue = 0;
...
// 炮：一次四向射线，写 attack/guard/control mask
const fillCannonRelations = (squareCodes, info, pieceAtSq, relCtx) => {
    const { r, c, pieceCode } = info;
    const isRed = pieceCode < 8;
    const bit = 1 << relCtx.pieceIndex;
    let mobilityValue = 0;
...
const calculatePackedSearchLeafRelations = (pieceState, aliveMask) => {
    ...
    let liveSlots = aliveMask >>> 0;
    while (liveSlots !== 0) {
        const bit = liveSlots & -liveSlots;
        const slot = 31 - Math.clz32(bit);
        liveSlots ^= bit;
...
const calculatePieceRelations = (board, piecesInfo, boardInfo) => {
    ...
    for (let i = 0; i < piecesInfo.length; i++) {
        const info = piecesInfo[i];
        relCtx.pieceIndex = info.pieceIndex;
...
const calculateStaticExchangeScore = (
...
const calculateTacticalValues = (piecesInfo, currentPlayer, boardInfo = null, board = null) => {
    ...
    if (collectUi) {
        boardInfo.checks = [];
        boardInfo.threatenedPieces = [];
        boardInfo.canCapture = [];
    }
    ...
    for (let ti = 0; ti < piecesInfo.length; ti++) {
        ...
        if (am === 0) continue;
        const firstAttacker = piecesInfo[lowestSetBitIndex(am)];
        const hasGuard = guardMask[sq] !== 0;
            ...
            if (collectUi) {
                ...
                while (m !== 0) {
                    ...
                    boardInfo.checks.push({
                        attacker: piecesInfo[31 - Math.clz32(bit)],
        ...
        if (!hasGuard) {
            ...
            if (collectUi) {
                if ((firstAttacker.pieceCode < 8) === (currentPlayer === SIDE_RED)) {
                    let m = am >>> 0;
                    while (m !== 0) {
                        const bit = m & -m;
                        const info = piecesInfo[31 - Math.clz32(bit)];
    ...
    // 安全值：将空控邻格是否被敌控
    if (!board || !squareCodes) return;
    for (let i = 0; i < piecesInfo.length; i++) {
        const general = piecesInfo[i];
        if ((general.pieceCode & 7) !== 1) continue;
        const isRed = general.pieceCode < 8;
        const enemyBits = isRed ? boardInfo.blackAttack : boardInfo.redAttack;
        const { r, c } = general;
        for (let d = 0; d < ORTH_DIRS.length; d++) {
            const nr = r + ORTH_DIRS[d][0];
            const nc = c + ORTH_DIRS[d][1];
            if (nc < 3 || nc > 5) continue;
            if (isRed ? (nr < 0 || nr > 2) : (nr < 7 || nr > 9)) continue;
            if (!squareCodes[nr * 9 + nc] && hasAttackBit(enemyBits, nr * 9 + nc)) {
...
class ZobristHasher {
    ...
    constructor() {
        ...
        this.evalInitiatorKeys = [
            Math.floor(seededRandom() * MAX_SAFE),
            Math.floor(seededRandom() * MAX_SAFE)
        ];
    }
...
const isCheck = (board, color, piecesInfo = null, boardInfo = null) => {
    ...
    if (boardInfo) {
        return side === SIDE_RED ? boardInfo.redIsInCheck : boardInfo.blackIsInCheck;
    }
    ...
    return runWithPieceState(board, () =>
        isCheckFromState(activeSearchPieceState, side)
    );
};
...
const checkGameState = (board, turn, piecesInfo = null, boardInfo = null) => {
    ...
    const opponent = sideToColor(side ^ 1);
    ...
    while (attackedTargets !== 0) {
        ...
        if ((targetCode & 7) === 1) {
            ...
            if (guards === 0) {
                ...
                const seeScore = calculateStaticExchangeScore(
...
const staticSearchEval = (searchInitiator, boardHash = 0) => {
    ...
    const combinedKey = cacheKey ^ activeSearchPieceState.evalVerificationHash;
...
const generateQuiescenceMoves = (currentPlayer, destination) => {
    const __t0 = searchContext.profile ? performance.now() : 0;
...
const alphaBeta = (
    ...
    for (let moveIndex = 0; ; moveIndex++) {
        ...
        if (canLmr) {
            ...
            const maxReduction = Math.min(searchContext.lmrMaxReduction, d - 2);
...
const fillRootSortHints = (pieceState, turn) => {
    ...
    let attacked = scratchLeafAttackedTargetMask >>> 0;
    while (attacked !== 0) {
        const targetBit = attacked & -attacked;
        const slot = 31 - Math.clz32(targetBit);
        attacked ^= targetBit;
...
const getBestMove = (
  ...
  if (searchContext.reportSearchProgress) {
  ...
  if (rootInCheck) {
    ...
    const pieceSquares = rootPieceState.pieceSquares;
    const n = collectOwnSlotsInScanOrder(rootPieceState, wantRed);
    for (let i = 0; i < n; i++) {
      const slot = scratchOwnScanSlots[i];
      const fromSq = pieceSquares[slot];
      const validDestinations = getValidMovesFromSq(fromSq, false, null);
  ...
  const rootTTKey = zobristHasher.ttKeyFromHash(rootHash, side);
  ...
  for (let currentDepth = 1; currentDepth <= maxDepth; currentDepth++) {
    ...
    sortRootMoves(rootMoves, side, rootBoardInfo, ttMove, null, null, [rootScores, rootSeqs]);
